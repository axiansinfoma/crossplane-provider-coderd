...
import (
	...
	apisCluster "github.com/axiansinfoma/crossplane-provider-coderd/apis/cluster"
	apisNamespaced "github.com/axiansinfoma/crossplane-provider-coderd/apis/namespaced"
	"github.com/axiansinfoma/crossplane-provider-coderd/config"
	"github.com/axiansinfoma/crossplane-provider-coderd/internal/clients"
	controllerCluster "github.com/axiansinfoma/crossplane-provider-coderd/internal/controller/cluster"
	controllerNamespaced "github.com/axiansinfoma/crossplane-provider-coderd/internal/controller/namespaced"
	"github.com/axiansinfoma/crossplane-provider-coderd/internal/features"
	"github.com/axiansinfoma/crossplane-provider-coderd/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Coderd").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-coderd"))
	if *debug {
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(scheme), "Cannot add cluster-scoped Coderd APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(scheme), "Cannot add namespace-scoped Coderd APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(scheme), "Cannot add CustomResourceDefinition to scheme")

	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		Scheme:           scheme,
		LeaderElection:   *leaderElection,
		LeaderElectionID: "crossplane-leader-election-provider-coderd",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Coderd APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Coderd APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-coderd:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Coderd controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Coderd controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Coderd controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Coderd controllers")
	}
