...
REGISTRY_ORGS ?= ghcr.io/axiansinfoma
IMAGES = $(PROJECT_NAME)
...
XPKG_REG_ORGS ?= ghcr.io/axiansinfoma
# NOTE(hasheddan): skip promoting on xpkg.crossplane.io as channel tags are
# inferred.
XPKG_REG_ORGS_NO_PROMOTE ?= ghcr.io/axiansinfoma
XPKGS = $(PROJECT_NAME)
...
xpkg.build.provider-coderd: do.build.images
...
# Checks out the docs of the Terraform provider release we generate from. The
# checkout is reused across runs, so it is moved onto the requested tag when
# TERRAFORM_PROVIDER_VERSION changes.
pull-docs:
	@if [ ! -d "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)/.git" ]; then \
		...
		git -C "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)" fetch -q --depth 1 --filter=blob:none origin "+refs/tags/v$(TERRAFORM_PROVIDER_VERSION):refs/tags/v$(TERRAFORM_PROVIDER_VERSION)" && \
		git -C "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)" checkout -q "v$(TERRAFORM_PROVIDER_VERSION)"; \
	fi
