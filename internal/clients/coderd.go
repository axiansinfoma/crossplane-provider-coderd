...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal coderd credentials as JSON"
	errMissingURL           = "coderd credentials must contain a non-empty \"url\""
	errMissingToken         = "coderd credentials must contain a non-empty \"token\""
)

// credentials is the JSON document expected in the Secret a ProviderConfig
// points at. It mirrors the configuration schema of the coderd Terraform
// provider.
type credentials struct {
	// URL of the Coder deployment, e.g. https://coder.example.com.
	URL string `json:"url"`
	// Token is an API token for the deployment. Most resource types require a
	// token with elevated permissions.
	Token string `json:"token"`
	// DefaultOrganizationID is the organization used for resources that do not
	// name one. Defaults to the first organization the token can see.
	DefaultOrganizationID string `json:"default_organization_id,omitempty"`
	// Headers are additional HTTP headers sent with every API request.
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if creds.URL == "" {
			return ps, errors.New(errMissingURL)
		}
		if creds.Token == "" {
		...
		if creds.DefaultOrganizationID != "" {
			ps.Configuration["default_organization_id"] = creds.DefaultOrganizationID
		}
		if len(creds.Headers) > 0 {
			ps.Configuration["headers"] = creds.Headers
		}
		return ps, nil
	}
}
